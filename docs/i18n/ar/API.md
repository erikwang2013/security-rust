...
مع بناء Release، تُجمَّع أنماط التعبيرات المنتظمة مسبقًا بشكل ساكن عبر `LazyLock` فلا تُعاد عملية التجميع بعد أول استخدام؛ وتكلفة فحص تعبير منتظم واحد في حدود مئات النانوثانية. وفحص جميع الكاشفات الـ 32 لمدخل من 26 بايت يستغرق نحو ~50 ميكروثانية في المرة (قياس محلي، ويزداد مع عدد التعبيرات وطول المدخل). مناسب لسيناريوهات الإنتاجية العالية (بوابات API وخطوط أنابيب السجلات).
