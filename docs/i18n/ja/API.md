...
Release ビルドでの実測（ノート PC、単一スレッド）では、単一検出器のスキャンが 1µs 前後、全 32 検出器でのスキャンは数十 µs/回です。値は CPU と入力長に依存します。高スループットのシナリオ（API ゲートウェイ、ログパイプライン）では、`scan_with()` で対象検出器を絞ることを検討してください。
